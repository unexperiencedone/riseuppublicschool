...
| 🔌 **REST API** | **81 endpoints**, JWT auth with refresh rotation, 7 roles, RBAC + row-level scoping |
...
| File | What it covers |
|---|---|
| **[`API_ENDPOINTS.md`](./API_ENDPOINTS.md)** | Every one of the 81 endpoints — method, path, auth, roles, request/response examples · endpoints still to be added · **all 10 third-party APIs to integrate**, with env vars and setup order |
| **[`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md)** | System topology · the middleware chain and why its order matters · **documented pipelines** for auth, admissions, payments, uploads, content publishing, attendance & results · data model · security posture · **serverless considerations** (§7A) · deployment pipeline · known gaps |
...
| **[`docs/DEPLOYMENT.md`](./docs/DEPLOYMENT.md)** | Step-by-step deployment: **two Vercel projects** (frontend + API) + Atlas + Cloudinary + Upstash, env vars, post-deploy checklist, cost breakdown, rollback |
...
│   ├── api/
│   │   └── index.js              ← Vercel entry point (src/server.js stays the local/long-running one)
│   └── src/
│       ├── app.js                ← middleware chain (order is load-bearing)
│       ├── server.js             ← bootstrap + graceful shutdown (npm run dev, any long-running host)
│       ├── config/               ← env, db (connection cached on globalThis), logger
│       ├── models/               ← 20 Mongoose models
│       ├── controllers/          ← 7 controllers
│       ├── routes/                ← auth · public · portal · admin · webhooks
│       ├── middleware/           ← auth · rbac · validate · rateLimit (Upstash-backed) · upload · audit · errors
│       ├── services/             ← email · sms · whatsapp · payment · storage (incl. Cloudinary signed uploads) · notification
...
└── web/                          ← Next.js 14 App Router
    ...
    ├── lib/                      ← config · api client · upload (direct-to-Cloudinary) · auth · calendar · fallback content
...
- ✅ API route table — **81 routes** resolve correctly
